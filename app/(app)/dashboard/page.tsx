...
import {
  TrendingUp, ShoppingCart, Package, AlertTriangle, DollarSign, Receipt,
} from "lucide-react";
  ...
  return [
    {
      ...
      value: formatCurrency(mockKpi.ventasMes),
      icon: Receipt,
    ...
    },
    {
      ...
      value: formatCurrency(mockKpi.comprasMes),
      icon: ShoppingCart,
    ...
    },
    {
      ...
      value: formatCurrency(mockKpi.gananciaMes),
      icon: DollarSign,
    ...
    },
    {
      ...
      icon: AlertTriangle,
      colorClass: alertas > 0 ? "text-chart-4" : "text-muted-foreground",
      bgClass: alertas > 0 ? "bg-chart-4/10" : "bg-muted",
      suffix: "productos",
    },
    {
    ...
    {
      label: "Ventas de Hoy",
      value: formatCurrency(mockKpi.ventasHoy),
      icon: TrendingUp,
...
export default function DashboardPage() {
  ...
  const cards = kpiCards(alertas, totalProductos);

  const ultimasVentas = [...facturas]
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={ventasMensuales} margin={{ top: 4, right: 4, left: -10, bottom: 0 }}>
                ...
                />
                <Tooltip
                  formatter={(value) => [`C$ ${Number(value ?? 0).toLocaleString()}`, ""]}
                  contentStyle={{
                    fontSize: 12,
                    borderRadius: 8,
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={220}>
              <PieChart>
                ...
                <Tooltip
                  formatter={(v) => `C$ ${Number(v ?? 0).toLocaleString()}`}
                  contentStyle={{
                    fontSize: 11,
                    borderRadius: 8,
                    border: "1px solid var(--color-border)",
          ...
          <CardContent className="flex flex-col gap-2.5">
            {topProductos.map((p, i) => (
              <div key={p.nombre} className="flex items-center gap-3">
                ...
                <span className="text-xs font-semibold text-chart-2 shrink-0">
                  C$ {p.ganancia.toLocaleString()}
                </span>
              </div>
            ))}
          </CardContent>
          ...
          <CardContent>
            <div className="flex flex-col divide-y divide-border">
              {ultimasVentas.length === 0 ? (
                <p className="text-sm text-muted-foreground py-2">Sin ventas recientes.</p>
              ) : (
                ultimasVentas.map((f) => (
                  <div key={f.facturaId} className="flex items-center gap-3 py-2.5 first:pt-0 last:pb-0">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {f.nombreCliente ?? "Cliente general"} · {f.metodoPagoNombre}
                      </p>
                    </div>
                    <span className="text-sm font-semibold shrink-0">
                      C$ {f.totalFactura.toLocaleString()}
                    </span>
                  </div>
                ))
              )}
          ...
          <CardContent>
            <div className="flex flex-col divide-y divide-border">
              {ultimasCompras.length === 0 ? (
                <p className="text-sm text-muted-foreground py-2">Sin compras recientes.</p>
              ) : (
                ultimasCompras.map((c) => (
                  <div key={c.compraId} className="flex items-center gap-3 py-2.5 first:pt-0 last:pb-0">
                ...
                .map((item) => (
                  ...
                  >
                    <p className="text-sm font-medium">
                      {item.nombreComercial} — {item.concentracion}
                    </p>
                    <Badge
                      variant={item.estado === "agotado" ? "destructive" : "outline"}
                      className={cn(
                        item.estado === "critico" && "border-chart-4 text-chart-4 bg-chart-4/10"
                      )}
                    >
                      {item.estado === "agotado"
                        ? "Agotado"
                        : `Stock crítico: ${item.cantidadDisponible}`}
                    </Badge>
                  </div>
                ))}
            </div>
